...
def run_ablation_experiment(
    ...
    - los mismos meses de train y test;
    - los mismos hiperparámetros;
    - la misma variable objetivo.

    Únicamente cambia el conjunto de características utilizado.
    """
    ...
    # Comparación incremental respecto
    # a la configuración anterior.
    for i, result in enumerate(results):

        if i == 0:
            result[
                "wape_improvement_vs_previous_pct"
            ] = None

            result[
                "rmse_improvement_vs_previous_pct"
            ] = None
...
def analyze_overstock(series, meta, forecasts, stock_in_file, params):
    ...
    for pid, fc in forecasts.items():
        ...
        if status == "sobre_stock":
            reasoning = (f"El stock actual alcanzaría para ~{int(coverage_days)} días considerando "
                        f"una demanda proyectada de {ref_month:.0f} u/mes ({trend}). Se estima un exceso de: {overstock_units} u"
                        + (f" (≈ S/ {excess_value:.0f} en inventario excedente))." if excess_value else "."))
        elif status == "bajo_stock":
            reasoning = (f"El stock actual alcanzaría para ~{int(coverage_days)} días considerando "
                        f"una demanda proyectada de {ref_month:.0f} u/mes "
                        f"Se recomienda reabastecer {understock_units} u para contar a stock suficiente para {target_days:.0f} días.")
        else:
            reasoning = (f"El stock actual alcanzaría para ~{int(coverage_days)} días, "
...
def inventory_policy_sensitivity(
    series,
    meta,
    forecasts,
    stock_in_file,
    params,
    target_days_options=(30, 45, 60),
):
    """
    Análisis de sensibilidad de la política de cobertura.

    Evalúa cómo cambian las necesidades de reposición y el exceso
    de inventario al utilizar objetivos de cobertura de 30, 45 y 60 días.

    No modifica el modelo XGBoost ni sus predicciones.
    """
    ...
    override_keys = {
        str(k): v
        for k, v in stock_override.items()
    }

    scenarios = []

    for target_days in target_days_options:
        ...
        for pid, fc in forecasts.items():
            ...
            # Misma demanda de referencia utilizada por analyze_overstock()
            ref_month = max(
                0.1,
                (fc_month + recent_month) / 2
            )

            ref_daily = ref_month / 30.0

            # Mismo criterio para obtener stock actual
            if str(pid) in override_keys:

                current_stock = float(
                    override_keys[str(pid)]
                )

                stock_source = "provisto"
            ...
            target_stock = int(
                round(
                    ref_daily * target_days
                )
            )

            reorder_units = max(
                0,
                target_stock - int(round(current_stock))
            )

            excess_units = max(
                0,
                int(round(current_stock)) - target_stock
            )
            ...
            if current_stock < target_stock:
                ...
                products_at_target += 1

            total_reorder_units += reorder_units
...
def inventory_backtest(
    ...
    Compara dos políticas:

    1. Baseline histórico:
        demanda esperada = media móvil de los últimos 3 meses.

    2. Propuesta:
        demanda esperada = promedio entre predicción XGBoost
        y media móvil de los últimos 3 meses.

    Ambas políticas utilizan la misma cobertura objetivo y
    parten del mismo inventario inicial.

    La demanda real del mes evaluado se utiliza únicamente
    para medir el resultado, nunca para tomar la decisión.
    """
    ...
    # Se utiliza exactamente el mismo TEST cronológico
    # que en la evaluación del modelo.
    ...
    # Predicciones generadas exclusivamente para TEST.
    ...
    # rm_3 ya fue construido con shift(1), por lo que
    # utiliza únicamente meses anteriores.
    ...
    # Política propuesta actualmente usada por el sistema.
    ...
    def _simulate(demand_column: str, strategy_name: str):
        ...
        for pid, g in test.groupby("product_id"):
            ...
            # Ambas estrategias comienzan exactamente
            ...
            for _, row in g.iterrows():
                ...
                # Stock objetivo para la política seleccionada.
                target_stock = (
                    expected_demand
                    / 30.0
                    * target_days
                ...
                # Reposición al inicio del periodo.
                reorder_units = max(
                    0.0,
                    target_stock - stock
                )
                ...
                # Aquí sí utilizamos la demanda real porque ya no
                # estamos tomando la decisión, sino evaluándola.
                # -------------------------------------------------
                ...
                # Inventario por encima de lo que hubiera sido
                # necesario para la cobertura objetivo real.
                excess_units = max(
                    0.0,
                    available_stock
                    - ideal_stock_actual
                ...
                # Sobrestock según el umbral operativo de 75 días.
                overstock_units = max(
                    0.0,
                    available_stock
                    - overstock_limit_actual
                ...
                rows.append({
                    ...
                    "excess_value_proxy":
                        (
                            ...
                            if excess_value_proxy
                            is not None
                            else None
                        ),
                })

                # El inventario final pasa al siguiente mes.
                stock = ending_stock
        ...
        # Inventario total al cierre de cada mes.
        monthly_inventory = (
            detail
        ...
        monthly = (
            ...
            .agg(
                ...
                reorder_units=("reorder_units", "sum"),
                ending_inventory=("ending_stock", "sum"),
            )
            .reset_index()
        )
    ...
    # ---------------------------------------------------------
    # Baseline: media histórica de los últimos tres meses
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Propuesta: XGBoost + política de reposición
    # ---------------------------------------------------------
    ...
    def _reduction_pct(baseline_value, proposed_value):
    ...
    return {
        ...
        "policy": {
            ...
            "initial_stock":
                (
                    "same_for_both_strategies_"
                    "based_on_previous_history"
                ),

            "replenishment_assumption":
                (
                    "monthly_periodic_review_"
                    "replenishment_available_"
                    "at_start_of_month"
                ),
        },
...
def inventory_backtest_sensitivity(
    full: pd.DataFrame,
    model,
    meta: dict,
    params: dict | None = None,
    ...
    """
    Ejecuta el backtesting de inventario bajo distintas
    políticas de cobertura objetivo.

    Permite comparar el impacto operativo de usar
    30, 45 y 60 días de cobertura.
    """
...
def run_full_analysis(df: pd.DataFrame, params: dict | None = None):
    ...
    inventory_sensitivity = inventory_policy_sensitivity(series,meta,forecasts,stock_in_file,params)
    inventory_backtesting = inventory_backtest(full,model,meta,params)
    inventory_backtest_scenarios = inventory_backtest_sensitivity(full,model,meta,params)
    ...
    return {
        ...
        "products": products,
        "inventory_policy_sensitivity": inventory_sensitivity,
        "inventory_backtest": inventory_backtesting,
        "inventory_backtest_sensitivity":inventory_backtest_scenarios,
        "timings": timings,
    }
